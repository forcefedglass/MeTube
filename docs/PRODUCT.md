...
· **Internal project codename:** MeTube

Slipgate is the public product identity of the project internally developed
under the MeTube codename — same application, same repository.
...
Slipgate exists for (2). It helps a person explore YouTube **without
...
## What Slipgate is not

- Not an ad blocker.
- Not a UI skin for YouTube.
- Not a political-balance tool. Slipgate deliberately does not classify
  content along political axes, does not measure "both-sides-ness", and does
  not optimize for ideological coverage.
- Not a re-ranker of YouTube's Home feed. Slipgate never consumes YouTube's
  own recommendations as input.
- Not a security tool. The walled-garden tagline refers to escaping the
  constraints of an algorithmically cultivated information environment —
  not compromising, attacking, or bypassing computer security.
...
5. **Present the Slipgate feed**, with "why this appeared" on every card and a
   feed autopsy view.
6. **Collect explicit feedback only.** Clicks and watch time are
   deliberately not signals. The user says what they think; Slipgate does not
   guess.
...
same subject from materially different evidenced positions, Slipgate offers
...
A user can open Slipgate, see material they would never have found via
